...
const PaymentHistory = () => {
  ...
  const fetchFranchises = async () => {
    try {
      const res = await api.get('/franchise-enquiry/public/stores');
      if (res.data.success) {
  ...
  const fetchInvoices = async (currentFilters = filters) => {
    try {
      ...
      if (currentFilters.franchiseId !== 'all') query.append('franchiseId', currentFilters.franchiseId);
  ...
  const clearFilters = () => {
    setPage(1);
    const resetFilters = { startDate: '', endDate: '', status: 'all', franchiseId: 'all' };
  ...
  const downloadBulkReport = async () => {
    try {
      ...
      if (filters.franchiseId !== 'all') query.append('franchiseId', filters.franchiseId);
      ...
      link.href = url;
      link.setAttribute('download', 'Payment_History_Report.pdf');
  ...
  return (
    <div className="bookings-container">
      ...
      <div className="bk-filters" style={{ display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'flex-end', marginBottom: '24px', background: '#fff', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
        ...
        <div style={{ flex: 1, minWidth: '150px' }}>
          <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#64748b', marginBottom: '4px' }}>Franchise</label>
          <select name="franchiseId" value={filters.franchiseId} onChange={handleFilterChange} className="bk-search-input" style={{ width: '100%', padding: '8px 12px' }}>
            <option value="all">All Locations</option>
            <option value="platform">Platform (No Franchise)</option>
            {franchises.map(f => (

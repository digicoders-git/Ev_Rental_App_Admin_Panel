...
import { Send, Bell, History, CheckCheck, Loader2, X, Trash2, Smartphone, Globe, Mail } from 'lucide-react';
...
const Notifications = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      setSending(true);
      await broadcastNotif({ title, message });
      setSentSuccess(true);
      setTitle(''); 
      setMessage('');
      fetchNotifications();
  ...
  return (
    <div className="notifications-page">
      <div className="page-header">
        ...
        <button className="btn btn-outline" onClick={handleMarkAll} disabled={markingAll || history.every(n => n.isRead)}>
            ...
            <form className="notification-form" onSubmit={e => e.preventDefault()}>
              <div className="form-group">
                ...
                <div className="input-with-icon">
                  ...
                  <input type="text" placeholder="e.g. Weekend Flash Sale - 20% Off" 
              ...
              <div className="form-group">
                ...
                <textarea placeholder="Type your message here. Keep it concise and engaging..." 
                  rows="5" value={message} onChange={e => setMessage(e.target.value)} />
              </div>

              <div className="form-info">
                <Globe size={14} />
                <span>This notification will be visible to all active app users.</span>
              </div>

              <button type="button" className="btn btn-primary btn-full" 
                onClick={handleSend} disabled={sending}>
                {sending ? <Loader2 size={18} className="spinner" /> : <><Send size={18} /> Send Broadcast Now</>}
              </button>
            </form>
          </div>
        </div>
        ...
        <div className="card history-section">
          ...
          <div className="history-list">
            {loading ? (
              ...
              history.map((item) => (
                <div key={item._id} className={`history-item ${!item.isRead ? 'unread' : ''}`} 
                  ...
                  <div className="history-item-body">
                    ...
                    <div className="history-item-footer">
                      <span className="notif-type">
                        {view === 'Sent' ? `Target: All Users (${item.recipient_count || 0} users)` : `Type: ${item.type || 'System'}`}
                      </span>
